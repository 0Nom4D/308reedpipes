...
class Reedpipe:
    ...
    def runEngine(self) -> None:
        ...
        self._drawReedpipeCurve()

    def _drawReedpipeCurve(self) -> None:
        pointAbscissas = np.array(self._pointAbscissas)
        radiusValues = np.array(self._radiusValues)
        negativeRadiusValues = np.negative(radiusValues)
        mySpline = make_interp_spline(pointAbscissas, radiusValues)
        mySndSpline = make_interp_spline(pointAbscissas, negativeRadiusValues)
        X_ = np.linspace(pointAbscissas.min(), pointAbscissas.max(), 500)
        Y_ = mySpline(X_)
        Z_ = mySndSpline(X_)
        plt.plot(X_, Y_)
        plt.plot(X_, Z_)
        plt.title("Graphical representation of a reedpipe")
        plt.xlabel("Reedpipe length in centimeter (cm)")
        plt.ylabel("Reedpipe radius in centimeter (cm)")
        plt.legend(['Up side of reedpipe', 'Bottom side of reedpipe'])
        plt.savefig("reedpipe_analysis.png")
